...
public class GrepTool extends ATool implements IGrepTool {
	...
	public String getMatchingLinesWithTrailingContext(int optionA,
		...
		for (int index : matchingIndexes) {
			...
			int maxIndex = Math.min(index + optionA, lines.length - 1);
	...
	public String getMatchingLinesWithLeadingContext(int optionB,
		...
		int prevIndex = -1;
		for (int index : matchingIndexes) {
			int minIndex = Math.max(index - optionB, 0);
	...
	public String getMatchingLinesWithOutputContext(int optionC,
		...
		for (int i = 0; i < matchingIndexes.length; i++) {
			...
			if (prevMaxIndex > -1) { // Make sure it's not the first match
				int smallestIndexInContext = matchingIndex - optionC;
			...
			int currentIndexInContext = matchingIndex - optionC;
			while (currentIndexInContext <= matchingIndex + optionC
			...
			prevMaxIndex = matchingIndex + optionC;
		}
			...
			if (!hasOption(args)) {
				...
				case "-A":
					try {
						...
						// Check if there is [FILES] arg, returns arg invalid err message if there is none.
						if (args.length < 4) {
							setStatusCode(ERR_CODE_INVALID_ARG);
							return String.format(ERR_MSG_INVALID_ARG);
						}
