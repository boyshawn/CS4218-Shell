...
public class MoveToolHelperMethodsTest {
	...
	public void setUp() throws Exception {
		...
		newFileInWD = new File(workingDir.toString()
				+ File.separator + "newFileInWorkingDir.txt");
		newFileInWD.createNewFile();

		newFileNotInWD = new File(tempDir.toString()
				+ File.separator + "newFileNotInWorkingDir.txt");
		newFileNotInWD.createNewFile();

		fileArgs = new String[5];

		File tempFile;

		for (int start = 0; start <= fileArgs.length - 2; start++) {
			tempFile = new File(Integer.toString(start) + ".txt");
			tempFile.createNewFile();
			fileArgs[start] = tempFile.getName();
		}

		fileArgs[4] = tempDir.getName();
	}
	...
	public void getReturnMessage_FileMoved_StatusCodeZero() {
		...
		final String returnMessage = moveTool.getReturnMessage(0);
	...
	public void getReturnMessage_FileNotFound_StatusCodeOne() {
		...
		final String returnMessage = moveTool.getReturnMessage(1);
	...
	public void getReturnMessage_MissingDestinationFile_StatusCodeMinusTwo() {
		...
		final String returnMessage = moveTool.getReturnMessage(-2);
	...
	public void getReturnMessage_NotADirectory_StatusCodeMinusThree() {
		...
		final String expectedMessage = "move: target '" + args[args.length - 1]
				+ "' is not a directory";
		final String returnMessage = moveTool.getReturnMessage(-3);
	...
	public void createFileObject_FileNotInWorkingDirectory() throws IOException {
		...
		final File fileObj = moveTool.createFileObject(
				newFileNotInWD.toString(), workingDir);
	...
	public void moveMultipleFiles() throws IOException {
		...
		moveTool = new MoveTool(fileArgs);

		final String returnMessage = moveTool
				.moveMultipleFiles(workingDir);
